...
resource "kong-gateway_plugin_confluent_consume" "my_confluent_consume" {
  enabled = true

  config = {
    bootstrap_servers = [
      {
        host = "example-bootstrap-server-host"
        port = 9092
    }]
    topics = [
      {
...
EOF
    ...
    cluster_api_key    = "example-api-key"
    cluster_api_secret = "example-api-secret"
  }
}
...
resource "kong-gateway_plugin_kafka_consume" "my_kafka_consume" {
  enabled = true
...
resource "kong-gateway_plugin_solace_consume" "my_solace_consume" {
  enabled = true
