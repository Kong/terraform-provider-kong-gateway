...
resource "kong-gateway_plugin_kafka_upstream" "my_pluginkafkaupstream" {
  ...
  config = {
    ...
    headers = {
      ...
      forward_http_headers_as_record_headers = false
      include_headers = [
        "..."
      ]
      name_mappings = {
      ...
      repeated_headers_behavior = "take_first"
    }
    keepalive         = 3
    ...
    new_kafka_async_producer                           = false
    producer_async                                     = false
    producer_async_buffering_limits_messages_in_memory = 3
    producer_async_flush_timeout                       = 0
    producer_async_health_failure_threshold            = 23
    producer_async_health_gating                       = true
    producer_async_health_probe_interval               = 434457
    producer_async_health_recovery_threshold           = 49
...
```terraform
import {
  to = kong-gateway_plugin_kafka_upstream.my_kong-gateway_plugin_kafka_upstream
  id = jsonencode({
...
```shell
terraform import kong-gateway_plugin_kafka_upstream.my_kong-gateway_plugin_kafka_upstream '{"id": "3473c251-5b6c-4f45-b1ff-7ede735a366d", "workspace": "team-payments"}'
```
