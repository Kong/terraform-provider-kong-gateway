...
func (r *PluginOpenidConnectResourceModel) ToSharedCreateOpenidConnectPlugin() *shared.CreateOpenidConnectPlugin {
	...
	if r.Config != nil {
		...
		var sessionRedisClusterNodes []shared.SessionRedisClusterNodes = []shared.SessionRedisClusterNodes{}
		for _, sessionRedisClusterNodesItem := range r.Config.SessionRedisClusterNodes {
			...
			sessionRedisClusterNodes = append(sessionRedisClusterNodes, shared.SessionRedisClusterNodes{
				IP:   ip,
				Port: port,
			})
		}
	...
	for _, protocolsItem := range r.Protocols {
		protocols = append(protocols, shared.CreateOpenidConnectPluginProtocols(protocolsItem.ValueString()))
	}
	var tags []string = []string{}
	for _, tagsItem := range r.Tags {
		tags = append(tags, tagsItem.ValueString())
	}
	var consumer *shared.CreateOpenidConnectPluginConsumer
	if r.Consumer != nil {
		id := new(string)
		...
		consumer = &shared.CreateOpenidConnectPluginConsumer{
			ID: id,
		}
	}
...
func (r *PluginOpenidConnectResourceModel) RefreshFromSharedOpenidConnectPlugin(resp *shared.OpenidConnectPlugin) {
	if resp != nil {
		if resp.Config == nil {
			r.Config = nil
		} else {
