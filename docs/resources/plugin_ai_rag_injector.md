...
page_title: "kong-gateway_plugin_ai_rag_injector Resource - terraform-provider-kong-gateway"
subcategory: ""
description: |-
  PluginAiRagInjector Resource
---

# kong-gateway_plugin_ai_rag_injector (Resource)
...
```terraform
resource "kong-gateway_plugin_ai_rag_injector" "my_pluginairaginjector" {
...
- `consumer_identifier` (String) The type of consumer identifier used for ACL checks. must be one of ["consumer_group", "consumer_id", "custom_id", "username"]
- `fetch_chunks_count` (Number) The maximum number of chunks to fetch from vectordb
- `filter_mode` (String) Defines how the plugin behaves when a filter is invalid. Set to `compatible` to ignore invalid filters, or `strict` to raise an error. This can be overridden per request. must be one of ["compatible", "strict"]
...
- `max_filter_clauses` (Number) Maximum number of filter clauses allowed
- `stop_on_failure` (Boolean) Halt the LLM request process in case of a vectordb or embeddings service failure
- `stop_on_filter_error` (Boolean) Default behavior when filter parsing fails (can be overridden per-request)
...
- `allow` (List of String) Consumer identifiers allowed access to this collection
- `deny` (List of String) Consumer identifiers denied access to this collection
...
- `allow` (List of String) Consumer identifiers allowed access (groups, IDs, usernames, or custom IDs based on consumer_identifier setting)
- `deny` (List of String) Consumer identifiers denied access (groups, IDs, usernames, or custom IDs based on consumer_identifier setting)
...
```terraform
import {
  to = kong-gateway_plugin_ai_rag_injector.my_kong-gateway_plugin_ai_rag_injector
  id = jsonencode({
...
```shell
terraform import kong-gateway_plugin_ai_rag_injector.my_kong-gateway_plugin_ai_rag_injector '{"id": "3473c251-5b6c-4f45-b1ff-7ede735a366d", "workspace": "team-payments"}'
```
