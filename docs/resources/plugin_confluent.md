...
resource "kong-gateway_plugin_confluent" "my_pluginconfluent" {
  ...
  config = {
    ...
    oauthbearer = {
      ...
      token_endpoint_tls_verify = false
    ...
    producer_async_health_failure_threshold            = 8
    producer_async_health_gating                       = false
    producer_async_health_probe_interval               = 237009
    producer_async_health_recovery_threshold           = 35
    producer_config_enabled                            = true
...
- `headers` (Attributes) Configuration for forwarding HTTP headers as Kafka record headers. (see [below for nested schema](#nestedatt--config--headers))
...
- `new_kafka_async_producer` (Boolean) Use the improved asynchronous Kafka producer, which batches messages more efficiently under high load. Only affects asynchronous mode. Messages without a key may be reordered across partitions; set a message key if ordering matters.
...
- `producer_async` (Boolean) Flag to enable asynchronous mode. Only takes effect when `producer_config_enabled` is true.
...
- `producer_async_health_failure_threshold` (Number) Number of consecutive authentication/authorization or connectivity failures before the producer is marked unhealthy and async requests are gated.
- `producer_async_health_gating` (Boolean) In asynchronous mode, fail incoming requests fast with HTTP 503 (instead of returning 200 and silently dropping the message) when the producer is sustainedly failing to authenticate with, or reach, the broker. Disabled by default.
- `producer_async_health_probe_interval` (Number) Interval in milliseconds between background recovery probes while the producer is unhealthy.
- `producer_async_health_recovery_threshold` (Number) Number of consecutive successful recovery probes required before the producer resumes accepting async requests.
- `producer_config_enabled` (Boolean) Honor the `producer_*` delivery settings (asynchronous mode, request acks/limits/retries). Historically the Confluent plugin ignored these and always produced synchronously; this flag defaults to false to preserve that behavior. Set to true to opt into the configured producer behavior.
...
- `exclude_headers` (List of String) Blocklist of HTTP header names to exclude from forwarding. Used when `forward_all_by_default` is `enabled`.
- `forward_all_by_default` (Boolean) When `false`, only headers listed in `include_headers` are forwarded. When `true`, all headers except those in `exclude_headers` are forwarded.
- `forward_http_headers_as_record_headers` (Boolean) Whether to forward HTTP headers as Kafka record headers.
- `include_headers` (List of String) Allowlist of HTTP header names to forward as Kafka record headers. Used when `forward_all_by_default` is `disabled`.
- `name_mappings` (Map of String) Map of HTTP header names to Kafka record header names. If an HTTP header name matches a key, the corresponding value is used as the Kafka record header name.
- `repeated_headers_behavior` (String) How to handle repeated HTTP headers: `concatenate_by_comma` joins values with a comma, `take_first` uses only the first value, `retain_duplicates` creates separate Kafka record headers for each value. must be one of ["concatenate_by_comma", "retain_duplicates", "take_first"]
...
```shell
terraform import kong-gateway_plugin_confluent.my_kong-gateway_plugin_confluent '{"id": "3473c251-5b6c-4f45-b1ff-7ede735a366d", "workspace": "team-payments"}'
```
