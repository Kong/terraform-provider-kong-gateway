...
- `oauth` (Attributes) OAuth 2.0 client configuration used to authenticate to Redis when `auth_provider` is set to `oauth`. (see [below for nested schema](#nestedatt--config--storage_config--redis--cloud_authentication--oauth))

<a id="nestedatt--config--storage_config--redis--cloud_authentication--oauth"></a>
### Nested Schema for `config.storage_config.redis.cloud_authentication.oauth`
