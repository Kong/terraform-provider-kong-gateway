...
- `scheme` (String) The client authentication scheme used when connection to an event broker. must be one of ["BASIC", "CLIENT_CREDENTIALS", "NONE", "OAUTH2"]
...
- `client_id` (String) The OAuth2 client ID used with `CLIENT_CREDENTIALS` authentication scheme when connecting to an event broker. Not Null
- `client_secret` (String) The OAuth2 client secret used with `CLIENT_CREDENTIALS` authentication scheme when connecting to an event broker. Not Null
- `eagerly_expire` (Number) Number of seconds before actual expiry when cached access tokens should be considered expired and proactively renewed. This helps prevent edge cases where tokens are rejected by Solace just as they expire, but setting this too high may lead to unnecessary token refreshes.
...
- `token_endpoint` (String) The OAuth2 token endpoint URL used to retrieve access tokens for the `CLIENT_CREDENTIALS` authentication scheme when connecting to an event broker. Not Null
