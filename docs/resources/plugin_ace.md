...
page_title: "kong-gateway_plugin_ace Resource - terraform-provider-kong-gateway"
subcategory: ""
description: |-
  PluginAce Resource
---

# kong-gateway_plugin_ace (Resource)
...
```terraform
resource "kong-gateway_plugin_ace" "my_pluginace" {
  ...
  config = {
    ...
    rate_limiting = {
      redis = {
        ...
        connect_timeout       = 554143355
        connection_is_proxied = true
        database              = 2
        host                  = "...my_host..."
        keepalive_backlog     = 699412344
        keepalive_pool_size   = 1355296038
        password              = "...my_password..."
        port                  = 17060
        read_timeout          = 571511457
        send_timeout          = 1258533784
...
### Nested Schema for `config.rate_limiting`
...
- `oauth` (Attributes) OAuth 2.0 client configuration used to authenticate to Redis when `auth_provider` is set to `oauth`. (see [below for nested schema](#nestedatt--config--rate_limiting--redis--cloud_authentication--oauth))

<a id="nestedatt--config--rate_limiting--redis--cloud_authentication--oauth"></a>
### Nested Schema for `config.rate_limiting.redis.cloud_authentication.oauth`
...
<a id="nestedatt--config--rate_limiting--redis--sentinel_nodes"></a>
### Nested Schema for `config.rate_limiting.redis.sentinel_nodes`
...
```terraform
import {
  to = kong-gateway_plugin_ace.my_kong-gateway_plugin_ace
  id = jsonencode({
...
```shell
terraform import kong-gateway_plugin_ace.my_kong-gateway_plugin_ace '{"id": "3473c251-5b6c-4f45-b1ff-7ede735a366d", "workspace": "team-payments"}'
```
