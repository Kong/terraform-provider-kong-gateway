...
resource "kong-gateway_plugin_solace_upstream" "my_solace_upstream" {
  enabled = true
  ...
  config = {
    session = {
      ...
      authentication = {
        scheme   = "BASIC"
        username = "your-static-solace-username"
        password = "your-static-solace-password"
      }
    }
    message = {
      destinations = [
        {
          name = "your-static-queue-name"
          type = "QUEUE"
        },
        {
          name = "your-static-another-queue-name"
          type = "QUEUE"
        }
      ]
