...
resource "kong-gateway_plugin_statsd" "my_pluginstatsd" {
  ...
  config = {
    ...
    prefix = "...my_prefix..."
    queue = {
      breaker_cooldown     = 367249.14
      concurrency_limit    = 1
      failure_threshold    = 620011
...
```terraform
import {
  to = kong-gateway_plugin_statsd.my_kong-gateway_plugin_statsd
  id = jsonencode({
...
```shell
terraform import kong-gateway_plugin_statsd.my_kong-gateway_plugin_statsd '{"id": "3473c251-5b6c-4f45-b1ff-7ede735a366d", "workspace": "team-payments"}'
```
