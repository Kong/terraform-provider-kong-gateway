...
- `private_key_jwt_include_kid` (Boolean) Whether to include the configured Key ID as the `kid` header in the JWT client assertion for `private_key_jwt` authentication. This field is ignored when `auth_method` is not `private_key_jwt`.
...
```shell
terraform import kong-gateway_plugin_upstream_oauth.my_kong-gateway_plugin_upstream_oauth '{"id": "3473c251-5b6c-4f45-b1ff-7ede735a366d", "workspace": "team-payments"}'
```
