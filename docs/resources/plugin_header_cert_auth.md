...
- `ca_certificates` (List of String) List of CA Certificates strings to use as Certificate Authorities (CA) when validating a client certificate. At least one is required but you can specify as many as needed. The value of this array is comprised of primary keys (`id`). Not Null
...
- `certificate_header_format` (String) Format of the certificate header. Supported formats: `base64_encoded`, `url_encoded`. Not Null; must be one of ["base64_encoded", "url_encoded"]
- `certificate_header_name` (String) Name of the header that contains the certificate, received from the WAF or other L7 downstream proxy. Not Null
