...
func (r *PluginSolaceLogResourceModel) RefreshFromSharedSolaceLogPlugin(ctx context.Context, resp *shared.SolaceLogPlugin) diag.Diagnostics {
...
func (r *PluginSolaceLogResourceModel) ToOperationsCreateSolacelogPluginRequest(ctx context.Context) (*operations.CreateSolacelogPluginRequest, diag.Diagnostics) {
	var diags diag.Diagnostics
...
func (r *PluginSolaceLogResourceModel) ToOperationsDeleteSolacelogPluginRequest(ctx context.Context) (*operations.DeleteSolacelogPluginRequest, diag.Diagnostics) {
	var diags diag.Diagnostics
...
func (r *PluginSolaceLogResourceModel) ToOperationsGetSolacelogPluginRequest(ctx context.Context) (*operations.GetSolacelogPluginRequest, diag.Diagnostics) {
	var diags diag.Diagnostics
...
func (r *PluginSolaceLogResourceModel) ToOperationsUpdateSolacelogPluginRequest(ctx context.Context) (*operations.UpdateSolacelogPluginRequest, diag.Diagnostics) {
	var diags diag.Diagnostics
...
func (r *PluginSolaceLogResourceModel) ToSharedSolaceLogPlugin(ctx context.Context) (*shared.SolaceLogPlugin, diag.Diagnostics) {
	...
	var ordering *shared.SolaceLogPluginOrdering
	if r.Ordering != nil {
		...
		ordering = &shared.SolaceLogPluginOrdering{
			After:  after,
			Before: before,
		}
	}
	...
	for partialsIndex := range r.Partials {
		...
		partials = append(partials, shared.SolaceLogPluginPartials{
			ID:   id1,
			Name: name,
			Path: path,
		})
	...
	for destinationsIndex := range r.Config.Message.Destinations {
		...
		destinations = append(destinations, shared.SolaceLogPluginDestinations{
			Name: name1,
			Type: typeVar,
		})
	}
	...
	var authentication *shared.SolaceLogPluginAuthentication
	if r.Config.Session.Authentication != nil {
		...
		var clientCredentials *shared.SolaceLogPluginClientCredentials
		if r.Config.Session.Authentication.ClientCredentials != nil {
			...
			clientCredentials = &shared.SolaceLogPluginClientCredentials{
				ClientID:      clientID,
		...
		scheme := new(shared.SolaceLogPluginScheme)
		if !r.Config.Session.Authentication.Scheme.IsUnknown() && !r.Config.Session.Authentication.Scheme.IsNull() {
			*scheme = shared.SolaceLogPluginScheme(r.Config.Session.Authentication.Scheme.ValueString())
		} else {
			scheme = nil
		}
		username := new(string)
	...
	session := shared.SolaceLogPluginSession{
	...
	config := shared.SolaceLogPluginConfig{
		Message: message,
		Session: session,
	}
	protocols := make([]shared.SolaceLogPluginProtocols, 0, len(r.Protocols))
	for _, protocolsItem := range r.Protocols {
		protocols = append(protocols, shared.SolaceLogPluginProtocols(protocolsItem.ValueString()))
	}
	var route *shared.SolaceLogPluginRoute
	if r.Route != nil {
		id2 := new(string)
		...
		route = &shared.SolaceLogPluginRoute{
			ID: id2,
		}
	}
	var service *shared.SolaceLogPluginService
	if r.Service != nil {
		id3 := new(string)
		...
		service = &shared.SolaceLogPluginService{
			ID: id3,
		}
	}
