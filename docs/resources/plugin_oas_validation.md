...
- `rfc9457_error_response` (Boolean) If set to `true`, interrupting non-gRPC HTTP validation failures are returned as `application/problem+json` using RFC 9457 top-level fields plus Kong extensions `message`, `errors`, and optional `more`. Requires `structured_errors` and `verbose_response` to both be set to `true`. When set to `false`, the legacy response format is preserved.
...
```terraform
import {
  to = kong-gateway_plugin_oas_validation.my_kong-gateway_plugin_oas_validation
  id = jsonencode({
...
```shell
terraform import kong-gateway_plugin_oas_validation.my_kong-gateway_plugin_oas_validation '{"id": "3473c251-5b6c-4f45-b1ff-7ede735a366d", "workspace": "team-payments"}'
```
