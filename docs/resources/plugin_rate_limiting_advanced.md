...
resource "kong-gateway_plugin_rate_limiting_advanced" "my_pluginratelimitingadvanced" {
  ...
  config = {
    ...
    throttling = {
      enabled     = true
      interval    = 888352.81
      queue_limit = 128480.13
      retry_times = 516089.69
    }
    window_size = [
      6.06
    ]
...
- `counter_key` (String) The key used to identify the counter for rate limiting. This can be based on consumer attributes such as `consumer.id`, `consumer.username`, or `consumer.custom_id`. Only applicable when `identifier` is set to `consumer`. must be one of ["consumer.custom_id", "consumer.id", "consumer.username"]
- `custom_key` (String) Overrides the computed rate-limiting key with a literal value for this request, regardless of `identifier` or `compound_identifier`.
...
- `identifier` (String) The type of identifier used to generate the rate limit key. Defines the scope used to increment the rate limiting counters. Note if `identifier` is `consumer-group`, the plugin must be applied on a consumer group entity. Because a consumer may belong to multiple consumer groups, the plugin needs to know explicitly which consumer group to limit the rate. must be one of ["consumer", "consumer-group", "credential", "header", "ip", "path", "principal", "route", "service"]
...
- `strategy` (String) The rate-limiting strategy to use for retrieving and incrementing the limits. Available values are: `local`, `redis` and `cluster`. must be one of ["cluster", "local", "redis"]
...
- `enabled` (Boolean) Determines if the throttling feature is enabled or not
- `interval` (Number) The period between two successive retries for an individual request (in seconds)
- `queue_limit` (Number) The maximum number of requests allowed for throttling
- `retry_times` (Number) The maximum number of retries for an individual request
...
```terraform
import {
  to = kong-gateway_plugin_rate_limiting_advanced.my_kong-gateway_plugin_rate_limiting_advanced
  id = jsonencode({
...
```shell
terraform import kong-gateway_plugin_rate_limiting_advanced.my_kong-gateway_plugin_rate_limiting_advanced '{"id": "3473c251-5b6c-4f45-b1ff-7ede735a366d", "workspace": "team-payments"}'
```
