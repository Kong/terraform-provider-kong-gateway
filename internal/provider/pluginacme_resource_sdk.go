...
func (r *PluginAcmeResourceModel) ToSharedCreateAcmePlugin() *shared.CreateAcmePlugin {
	var config *shared.CreateAcmePluginConfig
	if r.Config != nil {
		...
		if r.Config.StorageConfig != nil {
			...
			var redis *shared.CreateAcmePluginRedis
			if r.Config.StorageConfig.Redis != nil {
				...
				redis = &shared.CreateAcmePluginRedis{
					Database:     database,
			...
			var vault *shared.CreateAcmePluginVault
			if r.Config.StorageConfig.Vault != nil {
				...
				vault = &shared.CreateAcmePluginVault{
		...
		config = &shared.CreateAcmePluginConfig{
	...
	var protocols []shared.CreateAcmePluginProtocols = []shared.CreateAcmePluginProtocols{}
	for _, protocolsItem := range r.Protocols {
		protocols = append(protocols, shared.CreateAcmePluginProtocols(protocolsItem.ValueString()))
	}
	var tags []string = []string{}
	for _, tagsItem := range r.Tags {
		tags = append(tags, tagsItem.ValueString())
	}
	var consumer *shared.CreateAcmePluginConsumer
	if r.Consumer != nil {
		id := new(string)
		...
		consumer = &shared.CreateAcmePluginConsumer{
			ID: id,
		}
	}
	var consumerGroup *shared.CreateAcmePluginConsumerGroup
	if r.ConsumerGroup != nil {
		...
		consumerGroup = &shared.CreateAcmePluginConsumerGroup{
			ID: id1,
		}
	}
	var route *shared.CreateAcmePluginRoute
	if r.Route != nil {
		id2 := new(string)
		...
		route = &shared.CreateAcmePluginRoute{
			ID: id2,
		}
	}
	var service *shared.CreateAcmePluginService
	if r.Service != nil {
		id3 := new(string)
		...
		service = &shared.CreateAcmePluginService{
			ID: id3,
		}
	}
	out := shared.CreateAcmePlugin{
		Config:        config,
		Enabled:       enabled,
