...
- `llm_providers` (Attributes List) The provider config. Takes an array of `name`, `limit` and `window size` values. Not Null (see [below for nested schema](#nestedatt--config--llm_providers))
...
```shell
terraform import kong-gateway_plugin_ai_rate_limiting_advanced.my_kong-gateway_plugin_ai_rate_limiting_advanced '{"id": "3473c251-5b6c-4f45-b1ff-7ede735a366d", "workspace": "747d1e5-8246-4f65-a939-b392f1ee17f8"}'
```
