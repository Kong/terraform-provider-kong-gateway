...
- `include_body_in_opa_input` (Boolean) If set to true, the raw request body is included as input to OPA. Not supported for WebSocket protocols (ws/wss).
...
- `include_parsed_json_body_in_opa_input` (Boolean) If set to true and the `Content-Type` header of the current request is `application/json`, the request body will be JSON decoded and the decoded struct is included as input to OPA. Not supported for WebSocket protocols (ws/wss).
...
```shell
terraform import kong-gateway_plugin_opa.my_kong-gateway_plugin_opa '{"id": "3473c251-5b6c-4f45-b1ff-7ede735a366d", "workspace": "team-payments"}'
```
